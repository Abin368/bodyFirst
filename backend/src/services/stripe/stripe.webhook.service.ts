...
export class StripeWebhookService implements IStripeWebhookService {
  ...
  constructor(
    ...
    @inject(TYPES.OwnerPaymentRepository)
    private readonly _ownerPaymentRepo: IOwnerPaymentRepository,
    @inject(TYPES.StripeEventLogRepository)
    private readonly _eventLogRepo: IStripeEventLogRepository,
  ...
  async handleWebhook(body: Buffer, signature: string): Promise<void> {
    ...
    const inserted = await this._eventLogRepo.createIfNotExists(
      event.id,
      event.type,
    ...
    try {
      ...
      await this._eventLogRepo.updateStatus(
        event.id,
  ...
  private async handleCheckoutSessionCompleted(event: Stripe.Event): Promise<void> {
    ...
    try {
      const duration = parseInt(process.env.SUBSCRIPTION_DURATION_MS || '2592000000')
      ...
      await this._ownerProfileRepo.updateByUserId(
        ownerId,
        {
          ...
          subscriptionExpiry: new Date(Date.now() + duration),
      ...
      logger.info('Checkout session processed', {
        ownerId,
